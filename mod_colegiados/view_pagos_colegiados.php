<?php 
  ...
  include_once "../conf/conf.php";
    ...
    $sql_col = "SELECT `idColegiado`, `codigo_col`, `nom_colegiado`, `ape_paterno`, `ape_materno`, `dni`, `fec_nac`, `foto`, `telefono`, `email`, `password`, `direccion`, `lug_nacim`, `lug_labores`, `info_contacto`, `estado` FROM `colegiados` WHERE idColegiado = " . $_GET['idcol'];

    $db = $dbh->prepare($sql_col);
    $db->execute();
...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        <a class="btn btn-danger btn-md px-4" role="button" id="btnSalir">
          Salir
        </a>
      </li>      
    </ul>
            ...
            <div class="card">
              <div class="card-header">
                  <div class="row">
                    <div class="col-md-6">
                      <h3 class="card-title">
                        COLEGIADO:  <?php echo $data_colegiado->ape_paterno . " " . $data_colegiado->ape_materno . " " . $data_colegiado->nom_colegiado; ?></h3>
                    </div>
                    <div class="col-md-6">
                      <h3 class="card-title">CÓDIGO: <?php echo $data_colegiado->codigo_col; ?></h3>                      
                    </div>                    
                  </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="lista_pagos_colegiados_id" class="table table-bordered table-striped">
                  <thead>
                  <tr>
                    <th>FECHA</th>
                    <th>COMPROBANTE - Motivo</th>
                    <th>CARGO</th>
                    <th>ABONO</th>
                  </tr>
                  </thead>
                ...
                <?php  
                    ...
                    $sql = "SELECT `idPago`, `idColegiado`, `fecha_pago`, `comprobante_motivo`, `cargo`, `abono` 
                            FROM `pagos` 
                  ...
                  while($data = $db->fetch(PDO::FETCH_OBJ)):
                    $fechaPago = date("d/m/Y", strtotime($data->fecha_pago));
                ?>

                    <tr>
                      <td><?php echo $fechaPago; ?></td>
                      <td><?php echo $data->comprobante_motivo; ?></td>
  ...
  $( "#btnSalir" ).click(function() {
    if (confirm("Esta seguro de salir del sistema")) {
      ...
      $( document ).ready(function() {
        ...
        $('#lista_pagos_colegiados_id').DataTable({
          "paging": true,
          "processing": true,
          "lengthChange": false,
          "searching": true,
