...
// ********************* Fuera del switch **********************
			...
			if ($_POST["accion"]=='colegiados_sin_colegiatura') {
			...
			//	$sql = "SELECT idColegiado, codigo_col, nom_colegiado, ape_paterno, ape_materno, dni, fec_nac, foto, telefono, email, lug_nacim, lug_labores, info_contacto, estado FROM colegiados WHERE estado = 1 AND (ape_paterno like '%".$_POST["dni_codigo"]."%' OR dni = '".$_POST["dni_codigo"]."' OR codigo_col = '".$_POST["dni_codigo"]."')";
				...
				$sql = "SELECT C.idColegiado, 
											 ...
											 C.dni, 
											 C.fec_nac, 
											 C.foto, C.telefono, C.email, C.lug_nacim, C.lug_labores, C.info_contacto, C.estado 
								FROM colegiados C LEFT JOIN colegiatura CA
								ON C.idColegiado = CA.idColegiado
								WHERE estado = 1
								AND (ape_paterno like '%".$_POST["dni_codigo"]."%' OR dni = '".$_POST["dni_codigo"]."' OR codigo_col = '".$_POST["dni_codigo"]."') AND C.idColegiado NOT IN (SELECT idColegiado FROM colegiatura)";
		    ...
		    $db->bindValue(':password'		  	, md5( strval($valorRandom) ),PDO::PARAM_STR);
		    ...
		    $db->bindValue(':estado_exonerado', 0, 													PDO::PARAM_INT);
